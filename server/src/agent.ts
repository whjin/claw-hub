...
import OpenAI from 'openai'; // 添加OpenAI导入以获取正确的类型

// 定义与OpenAI SDK兼容的消息类型
...
function convertToOpenAIMessages(messages: Message[]): ChatCompletionMessageParam[] {
  return messages.map((msg) => {
    if (msg.role === 'tool') {
      ...
      // assistant消息包含tool_call_id的情况（虽然通常assistant消息应该包含tool_calls而不是tool_call_id）
      return {
        role: 'assistant',
  ...
  while (currentLoop < MAX_LOOP_COUNT) {
    ...
    try {
      ...
      const response = await llm.chat.completions.create({
        model: modelName,
        messages: [{ role: 'system', content: buildSystemPrompt() }, ...convertToOpenAIMessages(session.messages)],
